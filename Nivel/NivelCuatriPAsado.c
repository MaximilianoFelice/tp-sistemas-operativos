...
int main(int argc, char* argv[]) {
	...
	configNivel =
			config_try_create(
					...
					argv[1],
					"Nombre,orquestador,TiempoChequeoDeadlock,Recovery,ip,puerto,Caja1");
	...
	while ((arrCaja = config_try_get_array_value(configNivel, cajaAux))
		...
		if (posXCaja > rows || posYCaja > cols || posXCaja < 1
				|| posYCaja < 1) {
		...
		CrearCaja(&ListaItems, arrCaja[1][0], atoi(arrCaja[3]),
				atoi(arrCaja[4]), atoi(arrCaja[2]));
	...
	int tiempoChequeo = config_get_int_value(configNivel,
	...
	if ((sockOrq = socket(AF_INET, SOCK_STREAM, 0)) != 0) {
		...
		if (connect(sockOrq, (struct sockaddr*) &socketInfo, sizeof(socketInfo))
				== -1) {
			perror("Connect");
			exit(EXIT_FAILURE);
		}
	...
	// y los lanzamos
	if (pthread_create(&thr_interbloqueos, NULL, (void*) detectInterbloqueos,
			(void *) &parametros)) {
	...
	while (1) {
		...
		i = getSockChanged(&master, &temp, &maxSock, sockListener,
				&remoteAddress, &message, sizeof(message_t), logger);
		if (i != -1) {
			...
			case SALUDO:
				...
				log_info(logger, "Se agregó el personaje %c con el socket %d",
						message.name, i);

				enviaMensaje(i, &message, sizeof(message), logger,
						"Pos inicial");
				break;
			case POSICION:
			...
			case MOVIMIENTO:
				...
				if ((posItemX == posX) && (posItemY == posY)) {
					...
					for (contPj = 0; contPj < list_size(personajes); contPj++) {
						...
						if (tempAux->numSock == i) {
							list_add_new(tempAux->recursos, &(message.detail2),
									sizeof(message.detail2));
							break;
						}
					}
					...
					if (result >= 0) {
						...
						log_info(logger, "A %d se le da el recurso %c", i,
								message.detail2);
						...
						log_info(logger, "A %d se le niega el recurso %c", i,
								message.detail2);
			...
			case SALIR:
				for (contPj = 0; contPj < list_size(personajes); contPj++) {
					...
					if (tempAux->numSock == i) {
						...
						for (contRec = 0;
								contRec < list_size(tempAux->recursos);
								contRec++) {
						...
						list_remove_and_destroy_element(personajes, contPj,
								free);
...
_Bool tieneLoQueNecesito(personaje_t* a, personaje_t* b) {		//--Electrolitos
	...
	int contRec;
	char* levantadorA;
	for (contRec = 0; contRec < list_size(a->recursos) - (a->blocked ? 1 : 0);
			contRec++) {
	...
	while (1) {
		...
		for (; cantBloq >= 0; cantBloq--) { //(En el peor de los casos, tiene que asignar 2n-1 veces)
			...
			for (contPer1 = 0; contPer1 < list_size(personajes); contPer1++) {
				...
				if (!levantador1->marcado) {
					...
					for (contPer2 = 0; contPer2 < list_size(personajes);
							contPer2++) {
						levantador2 = list_get(personajes, contPer2);

						if (levantador2->marcado
								&& tieneLoQueNecesito(levantador2,
										levantador1)) {
							log_trace(logger, "Marque a %c",
									levantador1->chirimbolo);
							levantador1->marcado = true;
							break;
						}
					}
		...
		for (contPer1 = 0; contPer1 < list_size(personajes); contPer1++) {
			...
			if (!levantador1->marcado) {
				...
				log_trace(logger, "%c esta en Deadlock",
						levantador1->chirimbolo);
			}
		}
		...
		if ((list_size(bloqueados) > 1) && (parametros.recovery)) {
			...
			enviaMensaje(parametros.numSock, &header, sizeof(header), logger,
					"Envia header DDL");
			...
			while (list_size(bloqueados) != 0) {
				...
				enviaMensaje(parametros.numSock, &message, sizeof(message),
						logger, "Envia personajes DDL");
...
void notificarPersonajes() {
	...
	while (i < list_size(personajes)) {	//Cicla los personajes
		...
		enviaMensaje(personaje->numSock, &message, sizeof(message), logger,
