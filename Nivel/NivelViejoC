	...
	while (1) {
		if ((rv = poll(uDescriptores, 2, -1)) == -1) {
			...
			if (uDescriptores[1].revents & POLLIN) {
				i = 0;
				bytesLeidos = read(descriptorInotify, bufferInotify,
						TAM_BUFFER);
				while (i < bytesLeidos) {
			...
			if (uDescriptores[0].revents & POLLIN) {
				...
				case PL_CONEXION_PERS:
					...
					bool buscaPersonaje(pers_t* perso) {
				...
				case PL_MOV_PERSONAJE:
					...
					log_debug(logger,"<<< %s: El personaje %c solicito moverse",nom_nivel, movPersonaje.simbolo);
				...
				case PL_POS_RECURSO:
					...
					bool buscarRecurso(ITEM_NIVEL *item) {
						return ((item->id == (char) posConsultada->recurso)	&& (item->item_type == RECURSO_ITEM_TYPE));
					}
				...
				case PL_SOLICITUD_RECURSO:
					posConsultada = deserializarPregPosicion(payload);
					log_debug(logger,"<<< %s: Personaje %c solicita una instancia del recurso %c",nom_nivel, (char) posConsultada->simbolo,
							(char) posConsultada->recurso);
					...
					if (cantInstancias >= 0) {
						...
						bool buscarPersonaje(pers_t* personaje) {
							return (personaje->simbolo == posConsultada->simbolo);
						}
				...
				case PL_DESCONEXION_PERSONAJE:// Un personaje termino o murio y debo liberar instancias de recursos que tenia asignado
					...
					bool buscarPersonaje(pers_t* perso) {
						return (perso->simbolo == persDesconectado->simbolo);
					}
					pers_t *personajeOut = list_remove_by_condition(list_personajes, (void*) buscarPersonaje);
//					list_destroy_and_destroy_elements(personajeOut->recursos, free);
...
void levantarArchivoConf(char* argumento) {
	...
	for (i = 0; i < cantCajas; i++) {
		...
		CrearCaja(list_items, *lineaCaja[1], atoi(lineaCaja[3]),
				atoi(lineaCaja[4]), atoi(lineaCaja[2])); //*cajaRecursos[1], atoi(cajaRecursos[3]), atoi(cajaRecursos[4]), atoi(cajaRecursos[2]));
		pthread_mutex_unlock(&semItems);
	}
		...
		if (cantPersonajesActivos == 0) {
			...
			if (victimaAsignada == '0') { //No tiene victima =>reordenar viendo cual es la victima mas cercana
				...
				for (i = 0; i < list_size(list_personajes); i++) {
					...
					if ((char) persVictima->simbolo == victimaAsignada)
						break; //Cuando lo encuentro salgo del ciclo y listo
				}
	...
	while (1) {
		...
		if (cantPersonajes != 0) {
			...
			for (i = 0; i < cantPersonajes; i++) {
				fila = 0;
				for (j = 0; j < cantRecursos; j++) {
					if ((matSolicitud[i][j] - vecCajas[j].cantidadInstancias)
							<= 0) {
						fila++;
					}
			...
			if (cantPersonajesSatisfechos == cantPersonajes) {
				...
				if (recovery == 1) { //notificar a plataforma, entonces vecSatisfechos contendra -1-->el personaje quedo interbloqueado
					...
					enviarPaquete(sockete, &paquete, logger,
							"enviando notificacion de bloqueo de personajes a plataforma");
					pthread_mutex_unlock(&semSockPaq);
...
void liberarRecsPersonaje(char id) {
	...
	void desasignar(char* id1) {
		ITEM_NIVEL* itemAux;
		bool buscarRecurso(ITEM_NIVEL* item1) {
			return (item1->id == *id1);
		}
...
void eliminarPersPeroNoLiberarRecursos(char id) {
	bool buscarPersonaje(pers_t* perso) {
		return (perso->simbolo == (tSimbolo) id);
	}
	pers_t *personaje = list_remove_by_condition(list_personajes,(void*) buscarPersonaje);
